        ...
        qwant.api(qwant.routes.login, {
            ...
            .then(function (resolveAPI) {
                if (!resolveAPI.error) {
                    ...
                    //         browser.runtime.sendMessage({name: "qwant_extension_login"});
                    //     });
                    // }
                    browser.tabs.query({url: ["https://www.qwant.com/*", "https://boards.qwant.com/*"]}, function(tabs) {
                        ...
                        if (tabs.length === 0) {
                            browser.storage.local.set({lastLogAction: "in"});
                        }
                    });
                    updateIcon();
...
var userLogout    = function (eventsWorkers) {
    ...
    browser.storage.local.remove(["userExtension"]);
    ...
    //     });
    // }
    browser.tabs.query({url: "https://*.qwant.com/*"}, function(tabs) {
        for(var i=0; i<tabs.length; i++) {
            browser.tabs.sendMessage(tabs[i].id, {name: "qwant_extension_logout"});
        }
        if (tabs.length === 0) {
            browser.storage.local.set({lastLogAction: "out"});
        }
    });
    updateIcon();
...
var userSave      = function () {
    browser.storage.local.set({ userExtension: JSON.stringify(userData) });
};
var userLoad      = function (callback) {
    var userExtension, userWebsite, userJson, flag;
    browser.storage.local.get(["userExtension", "user"], function(items) {
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    switch (message.name) {
        case "account-loaded":
            browser.runtime.sendMessage({name: "popup_data", avatar: userData.avatar, username: userData.username});
            break;
    }
});

userLoad(() => {
    // Repeat instead of calling updateIcon method from qwant.js, because it's not loaded yet at startup on Edge.
