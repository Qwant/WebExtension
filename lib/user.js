...
var userLogin = function (data, eventsWorkers) {
    return new Promise(function (resolveLogin, rejectLogin) {
        console.log(qwant.routes.login);
        qwant.api(qwant.routes.login, {
            ...
            .then(function (resolveAPI) {
                ...
                if (!resolveAPI.error) {
                    //console.log('login:resolveApi:data: ', resolveAPI.data);
                    ...
                    //     });
                    // }
                    browser.tabs.query({url: ["https://www.qwantjunior.com/*", "https://www.qwantjunior.com/carnets*"]}, function(tabs) {
...
var userLogout    = function (eventsWorkers) {
    ...
    browser.tabs.query({url: "https://www.qwantjunior.com/*"}, function(tabs) {
        for(var i=0; i<tabs.length; i++) {
            ensureSendMessage(tabs[i].id, {name: "qwant_extension_logout"});
        }
        if (tabs.length === 0) {
