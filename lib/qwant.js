...
browser.runtime.onInstalled.addListener(function (details) {
    ...
    function setCurrentTabUrl(tabsArray) {
        if (tabsArray[0].url.match(/^https?:\/\/([a-zA-Z0-9_-]*)\.qwant\.(com|loc|dev|uat)/) !== null) {
            ...
            /**
             * TODO: Waiting for https://bugzilla.mozilla.org/show_bug.cgi?id=1403568 to be fixed
             * browser.tabs.create({
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                boards.getNotePreview({
                        ...
                        if (resolve.status !== "success") {
                            ...
                            browser.tabs.sendMessage(tabs[0].id, {name: "panel-note-advanced", data: resolve.data});
                        }
                    }, function (reject) {
                        ensureSendMessage(tabs[0].id, {
                            name   : "alert-display",
                            ...
                            browser.tabs.sendMessage(tabs[0].id, {name: "panel-note-enable"});
                        });
                    });
            });
            break;
                    ...
                    .then(function (resolve) {
                        if (resolve.status !== "error") {
                            ensureSendMessage(tabs[0].id, {
                                ...
                                user.getBoards()
                                    .then(function (resolve) {
                                        browser.tabs.sendMessage(tabs[0].id, {name: "panel-display-note", boards: resolve});
                                    });
                            });
                        } else {
                            ensureSendMessage(tabs[0].id, {
                    ...
                    .then(function (resolve) {
                        if (resolve.status === "error") {
                            ...
                            ensureSendMessage(tabs[0].id, {
                                ...
                                browser.tabs.sendMessage(tabs[0].id, {name: "panel-note-destroy"});
                            });
                        }
                    }, function (reject) {
                        ensureSendMessage(tabs[0].id, {
