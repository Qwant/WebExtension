...
browser.runtime.onInstalled.addListener(function (details) {
    function setCurrentTabUrl(tabsArray) {
        if (tabsArray[0].url.match(/^https?:\/\/www\.qwantjunior\.(com|loc|dev|uat)/) !== null) {
    ...
    var data = {
        type: 'firefox',
        context: manifest.version + '-junior'
    };
...
browser.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.match(/^https:\/\/www\.qwantjunior\.com/)) {
        browser.tabs.executeScript({
            code: "document.dispatchEvent(new CustomEvent('qwant_extension_loaded'));",
            runAt: "document_idle"
        }).then(function (result) {
            //console.log(result);
            browser.storage.local.get("lastLogAction").then(function (item) {
                //console.log(item);
                if (item && item['lastLogAction']) {
                    ...
                    if (lastLogAction === "in") {
                        ...
                        setTimeout(function() {
                            ensureSendMessage(tab.id, {name: "qwant_extension_logout"});
                            browser.storage.local.set({lastLogAction: ""});
                        }, 1000);
                    }
                }
...
browser.runtime.setUninstallURL('https://www.qwant.com/extension/feedback?v=' + manifest.version + '-junior');

browser.contextMenus.create({
    title   : 'Search Qwant Junior for "%s"',
    contexts: ["selection"],
    onclick : function (info) {
        var queryText = info.selectionText;
        browser.tabs.create({
            url: "https://www.qwantjunior.com/?q=" + queryText + "&client=ext-firefox-cm"
        });
    }
});
...
browser.omnibox.onInputEntered.addListener(function (text) {
    browser.tabs.query({
        ...
        browser.tabs.update(tabs[0].id, {
            url: "https://www.qwantjunior.com/?q=" + encodeURIComponent(text) + "&client=ext-firefox-ob"
        });
    });
});
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    ...
    switch (message.name) {
        case "popup_loaded":
            privacy.isEnabled(status => {
                ...
                browser.runtime.sendMessage({
                    ...
                    reload_enabled : _("tracking_protection_reload_enabled"),
                    reload_disabled: _("tracking_protection_reload_disabled")
                });
            });
            break;
        ...
        case "do_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://www.qwantjunior.com/carnets/user/" + userData.username + "?url=" + encodeURIComponent(tabs[0].url)
                });
            });
            break;
        case "qwant_website_login":
            ...
            updateIcon();
            browser.runtime.sendMessage({name: "account"});
            browser.tabs.query({url: "https://www.qwantjunior.com/*"}, function(tabs) {
