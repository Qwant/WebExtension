...
browser.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.match(/^https:\/\/.*\.qwant.com/)) {
        browser.tabs.executeScript({
...
/*browser.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.match(/^https:\/\/.*\.qwant.com/)) {
        browser.storage.local.get(["lastLogAction"], object => {
            if (object && object["lastLogAction"]) {
                var lastLogAction = object["lastLogAction"];
                if (lastLogAction === "in") {
                    setTimeout(function() {
                        ensureSendMessage(tab.id, {
                            name: "qwant_extension_login",
