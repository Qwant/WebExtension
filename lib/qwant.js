...
browser.runtime.onInstalled.addListener(function (details) {
    function setCurrentTabUrl(tabsArray) {
        if (tabsArray.length > 0 && tabsArray[0].hasOwnProperty('url') && tabsArray[0].url.match(/^https?:\/\/([a-zA-Z0-9_-]*)\.qwant\.com.*/) !== null) {
    ...
    if (details.reason === "install") {
        var currentTab = browser.tabs.query({active: true}, setCurrentTabUrl);
...
browser.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.match(/^https:\/\/(www|boards)\.qwant.com/)) {
        browser.tabs.executeScript(tabId, {
            ...
            browser.storage.local.get("lastLogAction", function (item) {
                if (item && item['lastLogAction']) {
                    ...
                    if (lastLogAction === "in") {
                        ...
                        setTimeout(function() {
                            browser.tabs.sendMessage(tab.id, {name: "qwant_extension_logout"});
                            browser.storage.local.set({lastLogAction: ""});
                        }, 1000);
                    }
                }
...
browser.webRequest.onBeforeRequest.addListener(function(details) {
    if (!details.url.match(/webext=true/)) {
        return {
            redirectUrl: details.url.replace('client=brz-moz', 'client=ext-edge-sb') + '&webext=' + manifest.version
        };
    }
...
browser.contextMenus.create({
    ...
    onclick : function (info) {
        ...
        browser.tabs.create({
            url: "https://www.qwant.com/?q=" + queryText + "&client=ext-edge-cm"
        });
    }
});
...
/* browser.omnibox.onInputEntered.addListener(function (text) {
    browser.tabs.query({
        ...
        browser.tabs.update(tabs[0].id, {
            url: "https://www.qwant.com/?q=" + encodeURIComponent(text) + "&client=ext-edge-ob"
        });
    });
}); */
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    switch (message.name) {
        ...
        case "link_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url:"https://boards.qwant.com/user/" + userData.username + "?client=ext-edge-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "link_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://www.qwant.com/all?tab=bookmarks&client=ext-edge-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
