    ...
    function setCurrentTabUrl(tabsArray) {
        if (tabsArray[0].url.match(/^https?:\/\/([a-zA-Z0-9_-]*)\.qwant\.(com|loc|dev|uat)/) !== null) {
            ...
            chrome.tabs.executeScript({
                code: "document.querySelector('.extension__opera__overlay__instructions').style.display = 'none';document.querySelector('.extension__overlay__thanks').style.display = 'block';"
            });
        }
        chrome.tabs.create({
...
chrome.contextMenus.create({
    ...
    onclick : function (info) {
        ...
        chrome.tabs.create({
            url: "https://www.qwant.com/?q=" + queryText + "&client=ext-opera-cm"
        });
    }
});
...
chrome.omnibox.onInputEntered.addListener(function (text) {
    chrome.tabs.query({
        ...
        chrome.tabs.update(tabs[0].id, {
            url: "https://www.qwant.com/?q=" + encodeURIComponent(text) + "&client=ext-opera-ob"
        });
    });
});
