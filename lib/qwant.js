...
browser.webRequest.onBeforeRequest.addListener(function(details) {
    if (!details.url.match(/webext=true/)) {
        return {
            redirectUrl: details.url.replace('client=brz-moz', 'client=ext-firefox-sb') + '&webext=' + manifest.version
        };
    }
...
browser.omnibox.onInputEntered.addListener(function (text) {
    browser.tabs.query({
        ...
        browser.tabs.update(tabs[0].id, {
            url: "https://www.qwant.com/?q=" + encodeURIComponent(text) + "&client=ext-firefox-ob"
        });
    });
});
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    switch (message.name) {
        ...
        case "link_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url:"https://boards.qwant.com/user/" + userData.username + "?client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "link_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://www.qwant.com/all?tab=bookmarks&client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
